...
<div class="container h-full mx-auto flex justify-center items-center">
	<div class="space-y-10 text-center flex flex-col items-center card p-2">
        ...
        <div class="flex flex-row justify-center items-center space-x-2 card p-5 bg variant-ghost-surface">
        {#if agentState === DEFAULT_AGENT_STATE}
    		<h4 class="h5 dynamic">Current State: <button type="button" class="btn variant-ghost-error">{agentState}</button> </h4>
        {:else}
            <h4 class="h5">Current State: <button type="button" class="btn variant-ghost-secondary">{agentState}</button></h4>
        {/if}
        ...
        <div class="flex flex-row justify-center items-center space-x-2 card p-5 bg variant-ghost-surface">
            <SlideToggle name="slider-label" bind:checked={advancedChecked} on:click={setAdvanced}>Simple/Advanced</SlideToggle>
            {#if advancedChecked == true}
                <SlideToggle name="slider-label" bind:checked={streamLogsChecked} on:click={renderAll}>Stream Logs</SlideToggle>
                <SlideToggle name="slider-label" bind:checked={followLogsChecked} on:click={followLogs}>Follow Logs</SlideToggle>
            {/if}

        </div>
        {#if advancedChecked == true}
        <div class="flex flex-row justify-center items-center space-x-2 card p-5 bg variant-ghost-surface">
            <SlideToggle name="slider-label" bind:checked={displayDatesChecked} on:click={renderAll}>Display Dates</SlideToggle>
            <SlideToggle name="slider-label" bind:checked={displayLogLevelChecked} on:click={renderAll}>Display Log Level</SlideToggle>
            <SlideToggle name="slider-label" bind:checked={displayAgentNameChecked} on:click={renderAll} >Display Agent Name</SlideToggle>
            <SlideToggle name="slider-label" bind:checked={displayModuleNameChecked} on:click={renderAll}>Display Module Name</SlideToggle>
        </div>
        {/if}
        

        <div class="flex flex-row justify-center items-center space-x-2 card p-2 bg variant-ghost-surface">
        {#if logs.length > 0 && advancedChecked}
            <button type="button" class="btn btn-sm variant-filled-warning" on:click={clearLogs}>Clear Logs</button>
            <button type="button" class="btn btn-sm variant-ghost-secondary m-2 p-2">Total Logs: {logs.length}</button>
            <button type="button" class="btn btn-sm variant-ghost-secondary m-2 p-2">Latest Date: {latestDate}</button>
        ...
        <div class="card p-2 w-full h-96 overflow-y-scroll" id="log-card">
            <Table regionHeadCell="text-center card bg variant-ghost-surface" source={tableData}></Table>
        </div>
    </div>
	</div>
</div>
